...
$config['db_name']    = "starbucks_twitter";
...
$sql = "SELECT * FROM tweets where approved=1 order by mysqldate desc limit 40";
   ...
   while($result_array = array_shift($rowset))
   {
      ...
      foreach($result_array as $key => $value)
      {
         ...
         $xml .= "\t\t<$key>";
 			...
 			if($key=="text") {
				$value = preg_replace('/#[^\s]*/i', '\1<font color=\"#F79DB6\">$0</font>', $value);
				
				$value = preg_replace('/@[^\s]*/i', '\1<font color=\"#FF6694\">$0</font>', $value);
				
				$value = preg_replace('/http:\/\/[^\s]*/i', '\1<font color=\"#AAAAAA\">$0</font>', $value);
